...
class MultiPageFormViewController: UIViewController, UIPageViewControllerDelegate {
    ...
    func doSubmit() {
        ...
        client.sendResponses(for: f, authorizedBy: f?.parentSubject.user, deviceID: "fake-device-id") { (error: Error?) -> Void in
    ...
    @IBAction func doMoveToPrevious() {
        ...
        if stepSequencer.moveToPrevious(withResponseRequired: false) {
            guard let fieldId = stepSequencer.currentField?.objectID else {
                return
            }
